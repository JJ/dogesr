...
```{r same.year}
knitr::kable(doges.years[doges.years$years == 0,]$doge, col.names=NULL)
```

And quite a few during 1 year or less

```{r one.year.or.less}
knitr::kable(unique(doges.years[doges.years$years <= 1,]),col.names=NULL)
```
...
```{r longest.doge}
knitr::kable(doges.years[which.max(doges.years$years),])
```
