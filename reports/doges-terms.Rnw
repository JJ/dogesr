...
<<r distribution, fig.height=4, echo = FALSE, fig.cap="This histogram reproduces, with a fixed bin size, Figure 1 in \\protect\\cite{smith2021long}.\\protect\\label{fig:histo}">>=
library(ggplot2)
ggplot(doges.years.all, aes(x=Years))+geom_histogram(binwidth=5)
@

<<r boxplot, echo = FALSE, fig.height=4,fig.cap="Boxplot of doge duration in office vs. century.\\protect\\label{fig:boxplot}">>=
