...
\address{$^{1}$ \quad Department of Computer Architecture and Technology, University of Granada; jmerelo@ugr.es}
...
event, which effectively resulted in term limits, as has already been reported by several authors. In this paper we examine the length of these tenures
and its evolution during the existence of the Venetian republic, following Smith et al. claim that specific events in Venetian history caused this \emph{shortening}, but also the dates, and possibly event or events that
...
with a certain economic position\rev{,} or at any rate chosen by the doge, \rev{provoked many changes in the social dynamics, as well as the institutional design}. \rev{Moreover}, by
...
``old'' families. \rev{Whether this constituted an actual closing or reduction of the amount of people eligible for office or an enlargement, is still under dispute \cite{roesch00}. However, by introducing a legal foundation for this eligibility, it certainly contributed to the definition of a noble or patrician class, which was by many authors considered the constitutional event of the Venetian republic \cite{roesch00}. }
...
\rev{The \emph{serrata} was not the last watershed event; some authors \cite{roesch00,chojnacki00} talk about a second and even a third \emph{serrata}, extending the concept to include legal changes that made the definition of nobility and other social castes evolve again \cite{chojnacki00}  }
...
\texttt{dogesr} \cite{dogesr}, we will, in this paper, see to
...
\emph{rectors} to a single month \cite{kunvcevic2018political}, and they could not be
...
given a pension for life \cite{wiel1891two}. In fact, he died a few days later,
...
\cite{loredan}, and he was chosen by the minimum needed
...
<<distribution, fig.pos="h!tbp",  echo = FALSE, fig.cap="This histogram reproduces, with a fixed bin size, Figure 1 in \\protect\\cite{smith2021long}.\\protect\\label{fig:histo}">>=
library(ggplot2)
...
<<serrata, echo=FALSE, fig.pos="h!tbp", fig.height=5, fig.cap="Box-and-whiskers plot of the span, in years, of doges in office pre and post 1329 (indicated with the pre.serrata variable).\\protect\\label{fig:serrata}">>=
...
Using Lanzante's test \cite{LANZANTE} from the \texttt{trend} R package,
...
the Serrata were enacted, please see also \cite{dogesr} for the
...
are lower than the ones published in \cite{smith2021long}, with the difference also being smaller.
...
introduction of the electoral college, as affirmed by \cite{smith2021long}. As a matter of fact, looking at Figure 2, we see
...
In this case, plotted in Figure \ref{fig:decline}, the difference in medians, of 1.5 years, is smaller; same happens
...
that is different to the one claimed by \cite{smith2021long}, that is, avoiding entrenchment in power and corruption.
...
A more sensitive analysis could become a future line of work. In their paper, \cite{smith2021long} examine also how the presence or absence of conflict could have an influence in the age of the elected doge; that could be one additional line of research, but, along the lines of the \cite{dogesr} report, how the position of the candidate in the social network could also play an additional role in this choice.

It would be interesting to check whether other republics with similar systems, such as the Vatican or the republic of Genoa, did have a similar informal term limit and what kind of factors influenced it. Further research will be needed to prove or disprove these hypotheses, and show similarities (or disparities) with the Venetian system.
