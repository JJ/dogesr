...
Let us first examine the claims about doges' terms using independent\rev{ly collected}
...
although the stress caused by his \rev{dismissal} might have contributed to \rev{his death}.
...
knitr::kable(head(doges.years.all[order(-doges.years.all$Years),],10),caption="Table that shows the top ten doges ranked by tenure \\protect\\rev{length, } the precise years where they acceded to power and left, and the total number of years.", row.names = F, booktabs=T)  %>%
    kable_styling(latex_options = c("striped","hold_position"))
@
