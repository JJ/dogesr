...
\rev{The single fact of changing from election in a popular assembly to instituting} an electoral college did not really have that effect. Electing a
person who could stay in power for many years did not \rev{change} the
possibility of having a person in your own family elected \rev{in a significant way}. Since the
pool was larger \rev{(any family of a certain standing in the city, a pool that changed continuously)}, the possibilities stayed more or less the same, and
close to zero. Someone in your family could or could not be elected
next, but since there were \emph{many} other possible candidates, it was really
...
terms. \rev{How this influenced the length of the terms} might be a future and interesting line of work, that might
be pursued in future papers.

A more sensitive analysis could \rev{also be pursued in the future}. In their paper, \cite{smith2021long} examine also how the presence or absence of conflict could have an influence in the age of the elected doge; that could be one additional line of research, but, along the lines of the \cite{dogesr} report, how the position of the candidate in the social network could also play an additional role in this choice.

It would be interesting to check whether other republics with similar systems, such as the Vatican, \rev{the republic of Ragusa} or the republic of Genoa, did have a similar informal term limit and what kind of factors influenced it. Further research, \rev{as well as data gathering}, will be needed to prove or disprove these hypotheses, and show similarities (or disparities) with the Venetian system.

\dataavailability{
This paper has been written using \rev{the literary programming system {\tt knitr}}, and all analysis performed
