...
republic. They propose that by not having a term limit \rev{imposed} by law, it is
...
reduced to persons that have a certain age \ref{and health state}. This also has the side
...
institutions with executive power, \rev{such as the council of Ten}. In general, term limits pursue
avoiding accumulation of power (and associated wealth) by elected
officials \rev{and their extended families}; in some cases \rev{what the law imposed were} extremely short.

\rev{For instance, } the republic of
...
reelected until two years \rev{after they had left office}. Limited terms also produced rotation
...
discussion. \cite{smith2021long} put the point of change in 1172, when new laws
...
knitr::kable(doges.one.year[order(doges.one.year$years),],col.names=NULL,caption="Doges whose tenure was within a natural year (indicated with 0 in the last column) or went from one year to the next. Since due to the imprecision in the data it is impossible to know the exact amount of months, they are grouped together in a single table.\\protect\\label{tab:oneyear}", row.names = F)
@
...
knitr::kable(head(doges.years.all[order(-doges.years.all$Years),],10),caption="Table that shows the top ten doges ranked by tenure and the precise years where they acceded to power and left, and the total number of years.", row.names = F)
@
