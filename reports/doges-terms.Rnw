...
\correspondence{Correspondence: jmerelo@ugr.es}
...
\Title{It's a doges' life: examining term limits in Venetian doges life
tenure}
...
place \cite{serrata}: seats in the Maggior Consiglio were
...
% Remember to change path to figures (although not totally sure it's reconstructed)
% Also convert to jpg if there's some change
...
<<boxplot, echo = FALSE, fig.pos="h!tbp", fig.cap="Boxplot of doge duration in office vs. century.\\protect\\label{fig:boxplot}">>=
doges.years.all$Century <- as.factor(doges.years.all$Century)
ggplot(doges.years.all, aes(x=Century,y=Years))+geom_boxplot()
@


<<serrata, echo=FALSE, fig.pos="h!tbp", fig.cap="Box-and-whiskers plot of the span, in years, of doges in office pre and post 1329 (indicated with the pre.serrata variable).\\protect\\label{fig:serrata}">>=
...
<<decline, echo=FALSE, fig.pos="h!tbp", fig.cap="Box-and-whiskers plot of the span, in years, of doges in office after 1329 and pre and post 1658 (indicated with the pre.decline variable).\\protect\\label{fig:decline}">>=
