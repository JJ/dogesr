...
introduction of the electoral college, as \rev{singled out} by \cite{smith2021long}\footnote{\rev{This does not mean that the laws introduced in 1172 themselves were not conducing to structural change, only that the change cannot be observed until much later, after more changes were introduced, because in the period between both events was characterized by a high level of conflict, which, again according to \cite{smith2021long}, implies selecting younger candidates with more \emph{èlan}. To a certain point, the observed shift in structural change confirms, rather than rectifies, the main thesis of that paper.}}
...
evidence \rev{that} term limits of doges of the Republic of Venice \rev{were actually enforced, albeit not in a formal way}. In their
paper they \rev{show there was a significant difference between terms before and after} 1172,
when the Maggior Consiglio was formed, \rev{which implied a shift from popular vote to a vote by an electoral college}. The \rev{statistical} analysis that we have
...
limited \rev{expected} lifespan.

\rev{U}sing \rev{a statistical technique called} change-point analysis\rev{which computes the point in a series where th average changes in a statistically significant way},
several algorithms support the \rev{moment in which the} election of Francesco Dandolo \rev{took place} as the
...
say that between 1328 and 1658 \rev{(when Pesaro was elected)} most nobles sitting in the final stages of
the electoral college were aware of \rev{it}, and so elected
older persons to the office, \rev{which resulted in doges} with a median \rev{remaining} life span of just 5 years.

How this informal measure in this reduced time period contributed to \rev{the}
stability \ref{of the state at large} is difficult to say. However, we can discuss that the fact
that it happened after the Serrata points to a rationale among electors
that is \rev{complementary} to the one\rev{s} claimed by \cite{smith2021long}, that is, avoiding entrenchment in power and corruption.
