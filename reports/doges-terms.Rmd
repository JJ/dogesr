...
In a previous version of this paper [@dogesr:vignette], we concluded that it was *not* the date and event proposed by Smith et al. the one that created a change in the attitude of the electoral college when choosing the next doge; we concluded that it was closer to the afore mentioned Serrata. In this paper we will try to use more precise statistical techniques to assess the precise point of change, and also check if there ware other events that produced changes during the history of the Republic
...
Using Lanzante's test [@lanzante] from the `trend` R package, this change occurs in the moment `r change.doge` took office. There seems to be also some significant change by the end of the republic; we will test again from this doge on looking for this change.

```{r change.point2, echo=FALSE}
after.dandolo <- tail(doges.all.years,cp.estimate$estimate)
cp.estimate.2 <- pettitt.test(tail(doges.all.years$years,cp.estimate$estimate))
change.doge.2 <- after.dandolo[as.integer(cp.estimate.2$estimate),]$doge
```

In this case, we use another change-point detection test which is more adequate in this case, Pettitt's test [@pettitt1979non]. This indicates the change point occurs with the `r change.doge.2` tenure.
